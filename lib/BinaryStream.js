...
BinaryStream.prototype = {
	...
	/**
	 * Split bundle at given offset into a separate file
	 */
	'separate': function( offset, filename ) {
		var newfd  = fs.openSync( filename, 'w+' );
		var BLOCK_SIZE = this.blockSize,
			buffer = new Buffer( BLOCK_SIZE ),
			readBytes = 0,
			iOfs = offset, oOfs = 0, iEnd = this.offset;

		// Copy bytes to new file
		while (iOfs < iEnd) {

			// Pick size of bytes to read
			readBytes = Math.min( BLOCK_SIZE, iEnd - iOfs );

			// Read and write
			// console.log("MERGE".magenta+" from["+otherStream.__fd+"]="+offset+", to["+this.__fd+"]="+this.offset+", range="+readBytes);
			fs.readSync( this.__fd, buffer, 0, readBytes, iOfs );
			fs.writeSync( newfd, buffer, 0, readBytes, oOfs );
		...
		// Truncate ourselves
		fs.ftruncateSync( this.__fd, offset );
		this.__syncOffset = offset;
		this.offset = offset;
